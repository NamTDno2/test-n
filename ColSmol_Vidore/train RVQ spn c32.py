# ==============================================================================
# METHOD 7 - Advanced RVQ Training on ColSmol Train Set
# Train RVQ codebooks using the official ColPali training dataset with ColSmol model.
#
# IMPROVEMENTS vs EMA baseline (train RVQ base c32.py):
#   ✅ SPN (Spherical Prototype Network) initialization per quantizer stage
#      → Better initial codebook geometry than random KMeans
#   ✅ Gradient-based AdamW fine-tuning (ema_update=False)
#      → Proper backprop through all quantizer layers
#   ✅ Quantizer Dropout with linear warmup
#      → Prevents over-reliance on early quantizers
#   ✅ Contrastive Triplet Ranking Loss (MaxSim-aware)
#      → Preserves retrieval ordering in compressed space
#   ✅ Wasserstein Optimal Transport Loss (Sinkhorn)
#      → Aligns geometric distribution of quantized vs original embeddings
...
EMB_DIM = 128   # ColSmol projection dimension (dim == 128)
...
# ── Hyperparameters ─────────────────────────────────────────────────────────────
RVQ_TRAINING_EPOCHS     = 40       # Max epochs for gradient fine-tuning
RVQ_TRAINING_BATCH_SIZE = 8192     # Gradient-step batch size (RTX PRO 6000 96GB)
RVQ_DROPOUT_P           = 0.5      # Quantizer dropout probability
RVQ_DROPOUT_WARMUP_EPS  = 10       # Epochs before dropout kicks in

N_TRAIN_PATCHES         = 1_700_000  # Total patches to collect across all shards
PATCHES_PER_SHARD       = 20_000    # Per-shard cap (domain balance)
ENCODE_BATCH_SIZE       = 16        # Images per ColSmol forward pass

# Spherical Prototype Network (SPN) config
SPN_EPOCHS         = 30
SPN_BATCH_SIZE     = 2048
SPN_LR             = 3e-3
SPN_HIDDEN_DIM     = 512
...
# Contrastive Triplet Ranking Loss config
RANK_LOSS_WEIGHT   = 1.0
RANK_MARGIN        = 0.2
RANK_NEG_SAMPLES   = 8

# Optimal Transport (Wasserstein / Sinkhorn) Loss config
OT_LOSS_WEIGHT     = 0.3
...
print(">>>> METHOD 7: ColSmol Advanced RVQ Training (SPN + Ranking + OT Losses)")
...
# ==============================================================================
print("Loading ColSmol model and processor...")
...
def collect_training_patches(shards, target=N_TRAIN_PATCHES,
    ...
    for shard_path in pbar:
        ...
        norms         = np.linalg.norm(shard_patches, axis=1, keepdims=True).clip(min=1e-8)
        shard_patches /= norms
...
# ==============================================================================
# LOSS FUNCTIONS (Gradient-based Advanced Training)
# ==============================================================================
...
def sinkhorn_log(log_alpha: torch.Tensor, n_iters: int = OT_SINKHORN_ITERS,
    ...
    """Numerically stable Sinkhorn algorithm in log domain"""
    N, M = log_alpha.shape
...
def wasserstein_ot_loss(x_orig: torch.Tensor, z_q: torch.Tensor,
    ...
    """Wasserstein OT loss measuring geometric distribution mismatch"""
    N = x_orig.shape[0]
    ot_n = min(N, 512)
...
def rvq_forward_with_dropout_and_ranking(
    ...
    """Forward pass with gradient flow and multi-task losses"""
    NQ       = len(model.layers)
...
class SphericalPrototypeNetwork(nn.Module):
    """MLP that learns optimal cluster centroid placement on the unit sphere"""
...
def train_spn_for_stage(residual_data: torch.Tensor, n_proto: int,
    ...
    """Train SPN to learn optimal codebook centroids for one RVQ residual stage"""
    ...
    best_proto   = None
    info         = {'max_entropy': float(np.log(n_proto))}
...
def initialize_rvq_codebooks_with_spn(
    ...
    """Initialize RVQ codebook weights per quantizer stage using SPN"""
    ...
    for k in range(NQ):
        ...
        # Compute residual_k+1 for the next stage
        with torch.no_grad():
...
# ==============================================================================
# MAIN TRAINING FUNCTION (SPN Init + Gradient Fine-tuning)
# ==============================================================================

def train_rvq_model_advanced(NQ: int, CB_SIZE: int, train_data: torch.Tensor,
                             dev: str, emb_dim: int) -> ResidualVQ:
    """Advanced RVQ training: SPN init + AdamW gradient descent with multi-task losses"""
    # Init RVQ with gradient descent enabled (ema_update=False)
    model = ResidualVQ(
        dim                     = emb_dim,
        num_quantizers          = NQ,
        codebook_size           = CB_SIZE,
        kmeans_init             = False,   # Replaced by SPN
        kmeans_iters            = 0,
        threshold_ema_dead_code = 2,
        commitment_weight       = 0.5,
        learnable_codebook      = True,
        ema_update              = False,   # Use gradient-based AdamW
    ).to(dev)
    ...
    # ── Phase 2: Gradient Fine-Tuning with Dropout & Multi-task Losses ───────
    print(f"  Starting Gradient Fine-Tuning | epochs: {RVQ_TRAINING_EPOCHS}")
    ...
    for epoch in range(RVQ_TRAINING_EPOCHS):
        ...
        # Quantizer Dropout: linearly ramp up after warmup epochs
        ...
        for attr_path in ['_codebook.embed', 'codebook', '_codebook.embed_avg']:
            try:
                ...
                if isinstance(obj, torch.Tensor):
                    ...
                    cb = F.normalize(cb.detach().float().cpu(), dim=-1)
                    break
...
def save_rvq_codebook(cbs_np, NQ, CB_SIZE, label):
    ...
    meta = {
        ...
        "trained_on"          : f"{N_TRAIN_PATCHES:,} patches - ColSmol train set (SPN)",
...
# Step 2: Encode images + domain-aware sampling using ColSmol model
_patches_np = collect_training_patches(_shards)
_train_data = torch.from_numpy(_patches_np)
del _patches_np; gc.collect()

# Step 3: Delete ColSmol encoder to free VRAM for RVQ training
del query_model
if torch.cuda.is_available():
    torch.cuda.empty_cache()
...
# Step 4: Train each RVQ config with advanced SPN + Gradient method, then save
_rvq_results = {}
